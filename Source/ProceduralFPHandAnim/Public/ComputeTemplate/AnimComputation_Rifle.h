...
struct FAnimComputation_Rifle : public IFPHandAnimComputeTemplate
{
	void Compute(const FFPHandAnimDataInput& In, FFPHandAnimDataOutput& Out, FFPHandAnimDataOutput CachedLastOutput) const override
	{
		...
		const float GlobalRotRoll = FMath::Clamp(In.LookInput.X * 5.f,-10.f, 10.f);
		const float GlobalRotYaw = FMath::Clamp(In.LookInput.X * 5.f,-5.f, 5.f);
		const float GlobalRotPitch  = FMath::Clamp(In.LookInput.Y * 5.f,-5.f, 5.f);
